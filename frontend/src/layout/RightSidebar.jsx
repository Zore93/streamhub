...
export default function RightSidebar({ recommendations = null }) {  const { user, logout } = useAuth();
  ...
  return (
    <aside
      ...
      className="fixed right-0 top-0 h-screen w-[280px] border-l border-zinc-800 bg-zinc-950/90 backdrop-blur-xl p-5 overflow-y-auto custom-scrollbar z-30"
    >
      {/* User block */}
      {user ? (
        ...
        <Link to="/pro" data-testid="pro-cta">
          <div className="pro-border-gradient rounded-xl p-5 mb-4 hover:scale-[1.02] transition-transform">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-heading font-bold text-zinc-50">Go PRO</span>
            </div>
            <p className="text-sm text-zinc-400 mb-3">Unlock premium videos, exclusive content & more.</p>
      ...
      {recommendations && recommendations.length > 0 && (
        <div data-testid="recommendations">
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-600 mb-3">Recommended</div>
            ...
            {recommendations.map((v) => (
              <Link key={v.id} to={`/watch/${v.id}`} className="flex gap-2 group" data-testid={`rec-${v.id}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-zinc-500 mt-1">{v.views} views</div>
                </div>
              </Link>
            ))}
          </div>
      ...
      {!recommendations && pkgs.length > 0 && (
        <div className="mt-4">
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-600 mb-3">Packages</div>
